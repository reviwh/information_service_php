...
final class CorruptionRepository
{
    ...
    if ($role === 'customer' && $this->isTokenValid($data['submitted_by'], $token)) {
      ...
      if (isset($id_card_desc['path']) && isset($complaint_report_desc['path'])) {
        $this->db->query(
          ...
          (reporter, no_telp, id_card, id_number, report_brief, complaint_report, status, submitted_by) 
          VALUES (:reporter, :no_telp, :id_card, :id_number, :report_brief, :complaint_report, 'pending', :submitted_by)"
        );
        ...
        $this->db->bind('report_brief', $data['report_brief']);
  ...
  public function update($id, $data, $field = null)
  {
    ...
    if (
      ...
      $this->db->query(
        "UPDATE {$this->table} 
            SET reporter=:reporter, no_telp=:no_telp, {$opt_set_query} 
                id_number=:id_number, report_brief=:report_brief
            WHERE id=:id"
      );
      ...
      if ($this->db->rowCount() > 0) {
        $result['code'] = 200;
        $result['message'] = "Corruption Complaint updated success.";
      } else {
        $result['code'] = 400;
        $result['message'] = "Corruption Complaint failed to update.";
      }
    } else {
      $result['code'] = 401;
      $result['message'] = "Unauthorized";
  ...
  public function delete($id, $data)
  {
    ...
    if (
      ...
      if ($this->db->rowCount() > 0) {
        $result['code'] = 200;
        $result['message'] = "Corruption Complaint deleted successfully";
        if ($id_card !== null) $this->deleteDoc($id_card);
        if ($complaint_report !== null) $this->deleteDoc($complaint_report);
      } else {
        $result['code'] = 400;
        $result['message'] = "Corruption Complaint failed to delete";
      }
    } else {
      $result['code'] = 401;
      $result['message'] = "Unauthorized";
  ...
  private function saveDoc($key, $value)
  {
    $result = array();
    $date = date('YmdHis');
    if (move_uploaded_file($value['tmp_name'], __DIR__ . '/../../api/storage/corruption_complaints/' . $key . '/' . $date . '_' . $value['name'])) {
      $result['path'] = "/storage/corruption_complaints/{$key}/{$date}_{$value['name']}";
    }
    return $result;
  }
