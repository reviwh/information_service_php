...
final class BeliefRepository
{
  ...
  public function update($id, $data, $field = null)
  {
    ...
    if (
      ...
      if ($this->db->rowCount() > 0) {
        $result['code'] = 200;
        $result['message'] = "Belief Control Complaint updated success.";
      } else {
        $result['code'] = 400;
        $result['message'] = "Belief Control Complaint failed to update.";
      }
    } else {
      $result['code'] = 401;
      $result['message'] = "Unauthorized";
  ...
  public function delete($id, $data)
  {
    ...
    if (
      ...
      if ($this->db->rowCount() > 0) {
        $result['code'] = 200;
        $result['message'] = "Belief Control Complaint deleted successfully";
        if ($id_card !== null) $this->deleteDoc($id_card);
        if ($complaint_report !== null) $this->deleteDoc($complaint_report);
      } else {
        $result['code'] = 400;
        $result['message'] = "Belief Control Complaint failed to delete";
      }
    } else {
      $result['code'] = 401;
      $result['message'] = "Unauthorized";
  ...
  private function saveDoc($key, $value)
  {
    $result = array();
    $date = date('YmdHis');
    if (move_uploaded_file($value['tmp_name'], __DIR__ . '/../../api/storage/beliefs_control/' . $key . '/' . $date . '_' . $value['name'])) {
      $result['path'] = "/storage/beliefs_control/{$key}/{$date}_{$value['name']}";
    }
    return $result;
  }
