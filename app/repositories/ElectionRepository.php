...
final class ElectionRepository
{
    ...
    if ($role === 'customer' && $this->isTokenValid($data['submitted_by'], $token)) {
      ...
      if (isset($id_card_desc['path']) && isset($complaint_report_desc['path'])) {
        ...
        if ($this->db->rowCount() > 0) {
          ...
          $result['code'] = 400;
          $result['message'] = "Post failed to create";
        }
      } else {
        $result['code'] = 400;
  ...
  public function update($id, $data, $field = null)
  {
    ...
    if (
      ...
      if ($this->db->rowCount() > 0) {
        $result['code'] = 200;
        $result['message'] = "Election Post updated success.";
      } else {
        $result['code'] = 400;
        $result['message'] = "Election Post failed to update.";
      }
    } else {
      $result['code'] = 401;
      $result['message'] = "Unauthorized";
  ...
  public function delete($id, $data)
  {
    ...
    if (
      ...
      if ($this->db->rowCount() > 0) {
        $result['code'] = 200;
        $result['message'] = "Election Post deleted successfully";
        if ($id_card !== null) $this->deleteDoc($id_card);
        if ($complaint_report !== null) $this->deleteDoc($complaint_report);
      } else {
        $result['code'] = 400;
        $result['message'] = "Election Post failed to delete";
      }
    } else {
      $result['code'] = 401;
      $result['message'] = "Unauthorized";
  ...
  private function saveDoc($key, $value)
  {
    $result = array();
    $date = date('YmdHis');
    if (move_uploaded_file($value['tmp_name'], __DIR__ . '/../../api/storage/election_posts/' . $key . '/' . $date . '_' . $value['name'])) {
      $result['path'] = "/storage/election_posts/{$key}/{$date}_{$value['name']}";
    }
    return $result;
  }
