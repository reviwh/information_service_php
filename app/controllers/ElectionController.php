...
class ElectionController extends Controller
{
  ...
  {
    $data = $this->repository('ElectionRepository')->getByUserId($id);
    http_response_code($data["code"]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (
        ...
        if ($_FILES['id_card']['type'] === 'application/pdf' && $_FILES['complaint_report']['type'] === 'application/pdf') {
          ...
          $data = $this->repository('ElectionRepository')->create($_POST);
          http_response_code($data["code"]);
      ...
      if ($field === null && $value === null) {
        if (
          ...
          if (
            ...
            $data = $this->repository('ElectionRepository')->update($id, $_POST);
            http_response_code($data["code"]);
            $response = new Response($data['message']);
            echo $response->send();
        ...
        if (isset($_POST['submitted_by'])) {
          $_POST[$field] = $value;
          $data = $this->repository('ElectionRepository')->update($id, $_POST, $field);
          http_response_code($data["code"]);
          $response = new Response($data['message']);
          echo $response->send();
        } else {
  ...
  public function delete($id)
  {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (isset($_POST['submitted_by'])) {
        $data = $this->repository('ElectionRepository')->delete($id, $_POST);
        http_response_code($data["code"]);
        $response = new Response($data['message']);
        echo $response->send();
      } else {
