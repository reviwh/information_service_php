...
class BeliefController extends Controller
{
  ...
  {
    $data = $this->repository('BeliefRepository')->getByUserId($id);
    http_response_code($data["code"]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (
        ...
        if ($_FILES['id_card']['type'] === 'application/pdf' && $_FILES['complaint_report']['type'] === 'application/pdf') {
          ...
          $data = $this->repository('BeliefRepository')->create($_POST);
          http_response_code($data["code"]);
      ...
      if ($field === null && $value === null) {
        if (
          ...
          if (
            ...
            $data = $this->repository('BeliefRepository')->update($id, $_POST);
            http_response_code($data["code"]);
            $response = new Response($data['message']);
            echo $response->send();
        ...
        if (isset($_POST['submitted_by'])) {
          $_POST[$field] = $value;
          $data = $this->repository('BeliefRepository')->update($id, $_POST, $field);
          http_response_code($data["code"]);
          $response = new Response($data['message']);
          echo $response->send();
        } else {
  ...
  public function delete($id)
  {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
      if (isset($_POST['submitted_by'])) {
        $data = $this->repository('BeliefRepository')->delete($id, $_POST);
        http_response_code($data["code"]);
        $response = new Response($data['message']);
        echo $response->send();
      } else {
